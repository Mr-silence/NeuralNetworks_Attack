...
# 读取Excel文件
excel_file = 'C:\\Users\\1\\Desktop\\副本1.xlsx'
df = pd.read_excel(excel_file)

# 选择要读取的列
column_name_child = '身份证号码'
column_name_mother = '父亲身份证号码'
...
df['父亲出生年'] = df[column_name_mother].str[6:10]
df['父亲出生月'] = df[column_name_mother].str[10:12]
df['父亲出生日'] = df[column_name_mother].str[12:14]
...
# 添加日期列到 DataFrame
df['父亲出生日期'] = father_birthdates
print(df['父亲出生日期'])


# 将"母亲出生日期"列转换为datetime类型
df['父亲出生日期'] = pd.to_datetime(df['父亲出生日期'], errors='coerce')
print(df['父亲出生日期'])
...
# 计算孩子出生日期和母亲出生日期的差值
df['年龄差'] = (df['孩子出生日期'] - df['父亲出生日期']).dt.days / 365
...
# 打印母亲怀孕的年龄
df['父亲在怀孕时的年龄'] = (df[['年龄差']]-0.75)
print(df['父亲在怀孕时的年龄'])

# 将数据列写入到 Excel 文件
output_file = 'C:\\Users\\1\\Desktop\\副本2.xlsx'  # 输出 Excel 文件名
df[['父亲在怀孕时的年龄']].to_excel(output_file, index=False, sheet_name='Sheet1', header=['父亲在怀孕时的年龄'])
